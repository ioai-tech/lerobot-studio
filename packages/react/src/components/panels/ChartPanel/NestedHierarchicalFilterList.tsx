...
import { getGroupSelectionState } from '@ioai/lerobot-studio-core';
import {
  filterFeatureNodes,
  getAllFeatureFilterItemIds,
  getFeatureNodeItemIds,
  type FeatureFilterNode,
...
function getNodeSelectionState(itemIds: string[], selectedIds: Set<string>): SelectionState {
...
export type NestedHierarchicalFilterListProps = {
  featureNodes: FeatureFilterNode[];
...
export const NestedHierarchicalFilterList = memo(function NestedHierarchicalFilterList({
  featureNodes,
  selectedIds,
  searchValue,
  onSearchChange,
  ...
  getItemColor,
  className,
}: NestedHierarchicalFilterListProps) {
  const allItemIds = useMemo(() => getAllFeatureFilterItemIds(featureNodes), [featureNodes]);
  const filteredNodes = useMemo(
    () => filterFeatureNodes(featureNodes, searchValue),
    [featureNodes, searchValue],
  );
    ...
    <div className={cn('w-full', className)} onMouseLeave={() => onHoverChange?.(null)}>
      ...
      <div className="max-h-[38vh] overflow-y-auto px-1 py-1">
        {filteredNodes.length === 0 ? (
          ...
          filteredNodes.map((featureNode) => {
            const featureItemIds = getFeatureNodeItemIds(featureNode);
            const featureState = getNodeSelectionState(featureItemIds, selectedIds);
              ...
              >
                <button
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    onSetItemsChecked(featureItemIds, featureState !== 'all');
                  }}
                  onMouseEnter={() =>
                    onHoverChange?.({
                      type: 'feature',
                ...
                >
                  <TriStateCheckbox
                    checked={featureState === 'all'}
                    indeterminate={featureState === 'partial'}
                  />
                  <span className="font-bold text-foreground truncate">{featureNode.label}</span>
                </button>

                {featureNode.subgroups.map((group: FilterGroup) => {
                  const groupItemIds = group.items.map((item) => item.id);
                  const groupState = getGroupSelectionState(group, selectedIds);

                  return (
                    <div key={`${featureNode.id}-${group.id}`} className="mb-0.5">
                      <button
                        type="button"
                        data-filter-row="true"
                        className="flex w-full items-center gap-2 rounded-md px-2 py-1 pl-4 text-left text-xs hover:bg-muted/50"
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                      ...
                      {group.items.map((item) => (
                        <button
                          ...
                          data-filter-row="true"
                          className={cn(
                            'flex w-full items-center gap-2 rounded-md px-2 py-1.5 pl-8 text-left text-[11px] hover:bg-muted/40',
