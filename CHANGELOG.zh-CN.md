...
- 在 Select 中支持批量裁剪 episode，并按运动自动建议保留区间。计算得到的区间仅在当前会话中保留，应用后才生效；导出沿用手动编辑的裁剪路径。

## 1.3.3

### 补丁变更

- 在编辑模式中支持裁剪 episode，并在导出时生效。同版本 v3.0 导出默认复制共享视频；勾选后才会物理去掉未使用片段。上调库入口 raw 体积预算以覆盖裁剪控件。
- 通过 overrides 将传递依赖 `js-yaml` 钉到 3.15.2 / 4.3.2、`hono` 钉到 4.13.7，清除 `npm audit`（GHSA-2883-xcg3-v3hh 以及 hono 4.13.5+ 安全修复）。这些包不是运行时应用依赖。
...
- 升级传递依赖 `fast-uri` 与 `qs`，清除 `npm audit`。
