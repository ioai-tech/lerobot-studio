...
describe('data source resource and download behavior', () => {
  ...
  it('resolves dropped directory files with explicit paths', async () => {
    const info = new File(['{"codebase_version":"v3.0"}'], 'info.json');
    const data = new File(['payload'], 'part.txt');
    const source = new FileListDirectoryDataSource([
      { file: info, path: 'dataset/meta/info.json' },
      { file: data, path: 'dataset/data/part.txt' },
    ]);

    await expect(source.listPaths()).resolves.toEqual(['meta/info.json', 'data/part.txt']);
    await expect(source.readText('meta/info.json')).resolves.toBe('{"codebase_version":"v3.0"}');
    source.clear();
  });
