...
describe('browser: shadcn Base UI primitives', () => {
  ...
  it('portals dialog overlays into #lerobot-root and traps Escape', async () => {
    ...
    // Centering uses Tailwind translate-* (CSS `translate`), not inline transform — avoids
    // Tailwind v4 stacking translate + transform:translate into a double offset.
    const contentEl = content as HTMLElement;
    expect(contentEl.className).toMatch(/top-\[50vh\]/);
    expect(contentEl.className).toMatch(/left-\[50vw\]/);
    expect(contentEl.className).toMatch(/translate-x-\[-50%\]/);
    expect(contentEl.className).toMatch(/translate-y-\[-50%\]/);
    ...
    // Vertically centered within the viewport (small dialog).
    const viewportMidY = window.innerHeight / 2;
    const contentMidY = contentRect.top + contentRect.height / 2;
    expect(Math.abs(contentMidY - viewportMidY)).toBeLessThan(8);
