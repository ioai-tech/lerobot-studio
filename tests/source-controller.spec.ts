...
describe('SourceController initialization', () => {
  ...
  it('initializes dropped directory files with explicit paths', async () => {
    const { deps, initialize } = createDeps();
    const controller = new SourceController(deps);
    const file = new File(['{}'], 'info.json');

    await controller.openDirectoryFiles([{ file, path: 'dataset-root/meta/info.json' }]);

    expect(initialize).toHaveBeenCalledTimes(1);
    expect(deps.setDatasetLabel).toHaveBeenCalledWith('dataset-root');
    expect(deps.completeTask).toHaveBeenCalledWith('open-dir');
  });
  ...
  it('detects a local archive by magic bytes when its filename has no extension', async () => {
    const { deps, initialize } = createDeps();
    const controller = new SourceController(deps);
    const file = new File([new Uint8Array([0x50, 0x4b, 0x05, 0x06])], 'dataset');

    await controller.openFile(file);

    expect(initialize).toHaveBeenCalledTimes(1);
    expect(archiveFactory.createArchiveDataSourceFromFile).toHaveBeenCalledWith(
      file,
      expect.any(Function),
      'zip',
    );
