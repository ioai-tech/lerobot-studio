...
export class SourceController {
  ...
  private getDirectoryLabelFromFiles(files: FileList | File[] | DirectoryFile[]): string {
    const firstFile = Array.from(files as unknown as ArrayLike<File | DirectoryFile>)[0];
    const relativePath =
      firstFile && 'file' in firstFile ? firstFile.path : firstFile?.webkitRelativePath || '';
    const label = relativePath.split('/')[0];
    return (
      label ||
      (firstFile && 'file' in firstFile ? firstFile.file.name : firstFile?.name) ||
      'directory'
    );
  }

  async openDirectoryFiles(files: FileList | File[] | DirectoryFile[]) {
    const taskId = 'open-dir';
  ...
  private async getLocalArchiveKind(file: File) {
    const kindFromName = getArchiveKindFromFile(file);
    if (kindFromName) return kindFromName;
    const bytes = new Uint8Array(await file.slice(0, 512).arrayBuffer());
    return getArchiveKindFromMagicBytes(bytes);
  }
  ...
  async openFile(file: File, sourceHandle?: FileSystemFileHandle) {
    ...
    try {
      ...
      const kind = await this.getLocalArchiveKind(file);
      const source = createArchiveDataSourceFromFile(
        file,
        this.createProgress(taskId, file.name),
        kind || undefined,
      );
