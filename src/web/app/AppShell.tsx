...
import { resolveDroppedItem, type SampleDataset } from '@/platform';
...
import { getDatasetDisplayName } from '@/core';
  ...
  useEffect(() => {
    ...
    const handleDrop = async (e: DragEvent) => {
      ...
      if (dropped.kind === 'directory-handle') {
        await controller.openDirectoryHandle(dropped.handle);
      } else if (dropped.kind === 'directory-files') {
        await controller.openDirectoryFiles(dropped.files);
      } else if (dropped.kind === 'file-handle') {
        await controller.openFile(dropped.file, dropped.handle);
      } else if (dropped.kind === 'file') {
        await controller.openFile(dropped.file);
      } else if (dropped.kind === 'unresolved-directory') {
        showToast(
          t('panels.welcome.dropDirectoryUnsupported', {
            name: dropped.name,
            defaultValue:
              'This browser could not read "{{name}}" as a folder. Use Open local directory or drop a .zip/.tar archive instead.',
          }),
          'warning',
        );
      }
