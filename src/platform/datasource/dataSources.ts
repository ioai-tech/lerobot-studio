...
import { isDirectoryFile, type DirectoryFile } from '../utils/droppedEntries';
...
import type { DataSource, ProgressHandler, ProgressInfo, LoadingPhase } from '@/core';
...
export class FileListDirectoryDataSource implements DataSource {
  ...
  constructor(files: FileList | File[] | DirectoryFile[]) {
    const selected = Array.from(files as unknown as ArrayLike<File | DirectoryFile>);
    const paths: string[] = [];
    for (const selectedFile of selected) {
      const { file, path } = isDirectoryFile(selectedFile)
        ? selectedFile
        : { file: selectedFile, path: selectedFile.webkitRelativePath || selectedFile.name };
