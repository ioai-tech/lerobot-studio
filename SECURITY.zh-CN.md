...
major 1 的最新 minor 版本会获得安全修复。较旧的 minor 可能提供协调升级路径而非 backport。开发快照、Fork、第三方部署与修改过的发行版仅尽力支持。

## 报告漏洞

请 **不要** 为安全漏洞公开开 Issue。

请从仓库 **Security → Advisories → Report a vulnerability** 提交私有 GitHub 安全公告，或发送邮件至 `io@io-ai.tech`，主题注明 “LeRobot Studio security report”。首次邮件中请勿包含密钥或敏感数据集内容。
...
Pull Request 会经 CI、依赖审查与 CodeQL 检查。OpenSSF Scorecard 定时分析会报告仓库级供应链发现。工作流权限默认为只读；仅上传安全结果或发布 release 的作业获得所需权限。Actions 使用版本标签而非 commit SHA（发布方提供主版本标签时用 `actions/checkout@v7`，否则用完整版本标签如 `astral-sh/setup-uv@v10.1.0`）。容器基础镜像使用不带 digest 的标签。Dependabot 跟踪 npm、GitHub Actions 与 Docker 的版本更新与安全更新：版本更新按月执行并先施加冷静期，安全更新在公告发布后立即开启，且每个生态的更新各自合并为单个 Pull Request。
